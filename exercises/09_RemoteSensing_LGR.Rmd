...
```{r packages}
pacman::p_load("rasterVis", "RColorBrewer", "landsat", "lattice", "latticeExtra", "rgl",
               "itcSegment", "terra", "raster")
```
...
data(nov3)
plot(nov3) #just the redband 

data(nov4) 
plot(nov4) #just the infrared
```
...
plot(imgData);plot(se,axes=F,add=T)
```
