...
#possion with envleope (simulations)
...
plot(redoak)
#test if the point pattern is more clustered than random (CSR) using quadrat.test
qt_re = quadrat.test(redoak, alternative = "clustered")
plot(qt_re)
print(qt_re)
#Calc nearest neigbor distances and plot as hist
hi_re = nndist(redoak)
hist(hi_re)
# Estitamte G
G_re <- Gest(redoak)
plot(G_re)
# estimate K
K_re_env <- envelope(redoak, Kest, correction = "border")
plot(K_re_env, . - pi * r ^ 2 ~ r)
K_re <- Kest(redoak , correction = "border")
plot(K_re, . - pi * r ^ 2 ~ r)
plot(K_re)


## quadrat test: low p-value means clustered point pattern
## nn_dist: values are clustered in low end 
## G: black line above red in some areas means clustered at these distances 
## K: blak line of observatiuon is constantly above the red of randomness
## K envelope: obs values diverge from the random background, they are clustered, from dist 1 to 2,5 is signficance clustering, aftwewards not so much 
```
