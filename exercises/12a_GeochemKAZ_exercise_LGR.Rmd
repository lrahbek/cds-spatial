...
kaz_geo <- read.csv("../data/KAZ_geo.csv") #it has long and lat cols so crs is 

#spatialise the kaz geo 
kaz_geo <- kaz_geo %>% 
  st_as_sf(coords = c("Long", "Lat"), crs = 4326) %>% #define sf 
  st_transform(crs = 32635) #transofmr to 2d
#how to define crs: 
  # the coordiniates indicate: they are degrees (they are small)
  #3D angular coordinates  
  #the most common 3D crs: 4326 (basically the same as wgs 84)
  #to make it 2D: st_transform, with the CRS rep WGS 84 UTM 35N, 32635
...
#.shp can use the sf package stuff immidiately, shape files should be zipped before being send 
#.csv is more flexible
#geojson is good? 
#kaz_sites: 
  #datum: WGS 84 
  #crs projection: UTM zone 35N
  #planar 2D immidiatley calculate distances because of this, 
  #2d is good for distances 


# See where the sites are
library(mapview)
mapview(kaz_sites) #mapview works imiditaley with spatial data
...
kaz_geo$X <- st_coordinates(kaz_geo)[,1] #it is projected, so it is no longer lat long, it is x y coords or northering estering 
...
#as X increase, moving east, the values go down
#as Y increase moving north the values increase 
```
...
predictions <- predict(m_trend, 
                       newdata = kaz_geo_miss, 
                       se.fit = TRUE)

kaz_geo_miss$OM <- predictions$fit
plot(kaz_geo_miss["OM"])
...
plot(variogram(OM_muns ~ 1, kaz_geo, 
               cloud = T, 
               cutoff = 10000)) #is 10 km, but unit is meters so 10,000
...
nugget <- 2.4 #where it starts 
psill <- 5-2.4 #eyeball partial sill: (psill) is 
range <- 4000 #where the x valut at sill value
