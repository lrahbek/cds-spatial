...
1. How many features (rows) does each layer (cols) contain and what kind of geometry are they?
```{r}
nrow(forests) #32, multi
nrow(playgrounds) #51, point
nrow(shelters) #19 , point
nrow(parks) #47, multi
```


2. What is the CRS value in these objects?
Avilable in header data; NA for playgrounds, 'ETRS89 / UTM zone 32N' for the others
...
plot(forests_geo, col = "green");plot(parks['geometry'], col = "hotpink", add = T)
...
class(elevation) #raster layer
class(aarhus) #raster brick 
...
elevation has the unit meter (to indicate elevation in the given cell)
aarhus 
