...
mun_sp<- readRDS("gadm36_DNK_2_sp.rds") # it's the gadm_... 2.rds dataset
mun_sf <- st_transform(st_as_sf(mun_sp), crs = 25832) #to be able to measure in meters 

# Plot so as to check correct location and complete coverage
plot(mun_sf$geometry)
...
unique(elections_data$Name)
# Join the election data with municipality polygons
elections <- sp::merge(mun_sf, elections_data, by.x = "NAME_2", by.y = "Name")

# Fix the missing counties 

# Map fraction of Socialdemokratie in 2015 to see no counties are missing
elections %>% 
  elecetions$pct_vote2015 %>% 
  filter("Party" == "A. Socialdemokratiet" ) %>%  # A.Socialdemokratie
