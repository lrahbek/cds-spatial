...
pacman::p_load("Rcpp", "fasterize", "readr", "sf", "tidyverse" )
```
...
my_token <- readr::read_file("../../tokens/mapboxAPI_token.txt")
...
## YOUR CODE HERE
library(dplyr)
#load all monuments with kommune info
viking <- readRDS("../data/viking.rds")
#filter Aarhus monuments
viking_aarhus <- viking %>% filter(NAME_2 == "Århus")
#check crs is the same
st_crs(viking_aarhus) == st_crs(nobel_iso_drive)
#transform monument crs to nobel is crs 
viking_aarhus <- st_transform(viking_aarhus, crs = 4326)
#make intersection obj where only the monuments within 12 minutes are saved and their time 
nobel_viking <- st_intersection(nobel_iso_drive, viking_aarhus)
#sum up distances 
table(nobel_viking$time)
#draw map with monuments 
leaflet() %>%
  addMapboxTiles(style_id = "outdoors-v11",
                 username = "mapbox") %>%
  addPolygons(data =nobel_iso_drive, 
...
library(tidyverse)
my_token <- readr::read_file("../../tokens/opencageAPI_token.txt")
...
```{r hospital-list}
hospitals <- c("Regionshospitalet Brædstrup in Brædstrup",
               "Regionshospitalet Grenaa in Grenaa",
               "Hammel Neurocenter",
...
```{r h-map, echo = FALSE}
#h2 <- readRDS("../data/hospitals.rds")
h3 <- read.csv("../data/midty_hospitals.csv")
leaflet() %>% 
  addTiles() %>% 
  addMarkers(h3$oc_lng, h3$oc_lat,data = h3, popup = h3$placename)
```
