...
distHaversine <- distHaversine(my_points)  # result is in meters
?distHaversine()
```
...
distCosine <- distCosine(my_points)  
```

### Meeus distance
```{r meeus}
 # Calculate Meeus distance
distMeeus <- distMeeus(my_points)                        
```
...
distVincentySphere <- distVincentySphere(my_points) 
# Calculate Vincenty ellipsoid distance
distVincentyEllipsoid <- distVincentyEllipsoid(my_points) 
...
distRhumb-distHaversine

# Calculate percent difference
(distRhumb-distHaversine)/distHaversine*100
(distCosine-distHaversine)/distHaversine*100
(distMeeus-distHaversine)/distHaversine*100
```
...
nobel <- nobel %>% st_as_sf(coords = c('longitude', 'latitude'), crs = 4326)

nobel <- st_transform(nobel, crs = 25832)
