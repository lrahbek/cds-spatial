...
hist(vals)
# Real values are in range 0 to 2000+
# NA = -9999 (these values are missing values that has been assigned the value -9999 as they are significantly outside the expected range of the data)
...
# Set up the matrix (we wish to replace values between -10000 and 0 with NA)
...
# Load the mounds dataset
mounds <- read_sf("../data/KAZ_mounds.shp") # .shp = vector

# Verify that the datasets have the same CRS
plot(elevation_reclass);plot(mounds$geometry, add = T)

# Create bounding box around the mounds
mounds_bb = st_make_grid(mounds, n = 1)
plot(elevation_reclass);plot(mounds$geometry, add = T);plot(mounds_bb, add = T)

# Crop the elevation_reclass layer by the mounds point object
elev <- crop(elevation_reclass, mounds)
plot(elev);plot(mounds$geometry, add = T);plot(mounds_bb, add = T)
...
survey <- read_sf("../data/KAZ_units.shp") # .shp = vector
survey

# Compute the area of the survey
areas <- st_area(survey)
class(areas) # unit --> therefore, we need to unclass
...
plot(elevation);plot(st_geometry(survey_big), add = T)
# I'm using elevation and not elev, as the geometry of survey_big is bigger than the bounding box from before
...
# Plot the elev_mask -- this is a raster!
plot(elev_mask)
```
...
east <- brick("../data/KazE.tif") # brick because it's a multi-band raster
east # 10x10 meters resolution
...
# aggregate to reduce resolution by the factor of 10
...
ncell(east_small) # 23,940

# if I were to create a ML pipeline, it's a great idea to make the image smaller to test the code (to decrease the computational complexity) --> then if it works, you can make it bigger again
...
plot(mounds["elev"])
hist(mounds$elev) # the mounds are most commonly found around 450 meters from sea level
...
elevation
prominence <- raster("../data/prominence1500m.tif") # .tif = raster

# Plot prominence
plot(prominence)

# Function f with 2 arguments and the raster math to select specific elevation range and prominence values
f <- function(rast1, rast2) {rast1 > 400 & rast1 < 650 & rast2 > 60}

# Align the extent of the two rasters with crop()
plot(elevation_reclass);plot(prominence, add=T) # as elevation_reclass is bigger, we crop by prominence
...
plot(prom_el_overlay);plot(mounds, add = T)
plot(prominence);plot(mounds, add = T)

# extract prominence and strategic position information at mounds locations
mounds$prominence <- raster::extract(prominence, mounds)
# remember that the results are in %
...
# black dots = all mounds (with reduced sizes, see cex)
# blue dots = most prominent mounds

# Make an object of 25 mounds with the highest prominence value
mounds %>% 
