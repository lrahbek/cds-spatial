...
# load household data
hh <- read.csv2("../data/households2020_2021.csv", header = F, fileEncoding="windows-1252")

# create a summary of households per municipality
hh_mun <- hh %>% 
  group_by(V2) %>% 
  summarize(sum20=sum(V3), sum21 = sum(V4)) %>% 
  rename(municipality = V2, households20 = sum20, households21 = sum21)

# check total households
sum(hh_mun$households20)
sum(hh_mun$households21)
...
hh_nocar <- hh_nocar %>% 
  mutate(nc2020pct = nc2020/households20*100, 
         nc2021pct = nc2021/households21*100)
...
munic_sf$NAME_2[munic_sf$NAME_2 == "Århus"] <- "Aarhus"

non_match <- munic_sf %>% dplyr::anti_join(hh_nocar, by = c("NAME_2"="municipality")) 
...
no_cars_pct_mun  <-  munic_sf %>% dplyr::inner_join(hh_nocar, by = c("NAME_2"="municipality")) # this preserves the spatial dimension!
```
...
regions_sf <- st_transform(st_as_sf(regions), crs = 25832)

midtjylland <- regions_sf[regions_sf$NAME_1 == "Midtjylland",]

no_cars_pct_midt <- st_intersection(midtjylland, no_cars_pct_mun)
...
## read in h file saved in 05b
h1 <- read.csv("../data/midty_hospitals.csv")
##transform to sf
h_sf <- st_as_sf(h1[-17,], coords = c("oc_lng", "oc_lat"), crs = 4326) 
...
# transform muni and car info to same crs as h and keep only municiplaities where 25% or above without cars 
no_cars_pct_midt_4326 <- st_transform(no_cars_pct_midt, crs = 4326)
no_cars_pct_midt_4326_25 <- no_cars_pct_midt_4326 %>% dplyr::filter(nc2021pct >= 25)

no_cars_h <- st_difference(no_cars_pct_midt_4326_25, st_union(cycling_isos))

leaflet() %>% 
  addMapboxTiles(style_id = "streets-v11", username = "mapbox") %>% 
  addPolygons(data = no_cars_h)
